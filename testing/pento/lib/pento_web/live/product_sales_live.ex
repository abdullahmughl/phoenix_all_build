...
defmodule PentoWeb.ProductSalesLive do
  ...
  def assign_product_names(socket) do
  ...
  def assign_dataset(%{assigns: %{product_names: product_names}} = socket) do
    assign(socket, :dataset, new_scatter_plot_dataset(product_names))
  end

  def assign_chart(%{assigns: %{dataset: dataset, product_names: product_names}} = socket) do
    socket
